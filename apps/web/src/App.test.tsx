...
describe("console navigation", () => {
  ...
  it("returns to the login screen after a 401 auth error", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(new Response(JSON.stringify({
      status: "authenticated", username: "admin", permissions: ["*"],
    }), { status: 200 })));
    render(<MemoryRouter initialEntries={["/"]}><App /></MemoryRouter>);

    expect(await screen.findByRole("link", { name: "验证工作台" })).toBeInTheDocument();
    window.dispatchEvent(new CustomEvent("agentbridge:auth-error", { detail: { status: 401 } }));

    expect(await screen.findByRole("heading", { name: "登录控制台" })).toBeInTheDocument();
    expect(screen.queryByRole("link", { name: "验证工作台" })).not.toBeInTheDocument();
  });
});
