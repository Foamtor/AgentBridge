...
export function PluginPlayground() {
  ...
  async function send() {
    ...
    try {
      ...
      }, onError: (reason) => setError(String(reason)) }, controller.signal);
    } catch (reason) {
  ...
  return <main className="plugin-playground">
    <header className="playground-header"><div><p className="eyebrow">{copy.live} / AgentBridge</p><h1>{copy.title}</h1><p>{copy.subtitle}</p></div>{selectedRun ? <div className="active-run"><span>{copy.run}</span><code>{selectedRun.run_id}</code><span>{copy.status}</span><strong>{selectedRun.status}</strong></div> : null}</header>
    <div className="playground-grid">
      <HistoryRail copy={copy} runs={runs} selectedRunId={selectedRun?.run_id} search={search} status={status} summary={summary} onSearch={setSearch} onStatus={setStatus} onSelect={(run) => void selectRun(run)} onRefresh={() => void refreshHistory()} />
      <div className="playground-main">
        <RequestComposer copy={copy} request={request} extraText={extraText} routes={routes} busy={busy} error={error} copied={copied} onRequest={setRequest} onExtraText={setExtraText} onSend={() => void send()} onCancel={() => void cancel()} onDoubleFire={() => void doubleFire()} onNewThread={() => setRequest((current) => ({ ...current, thread_id: newThreadId() }))} onLoad={loadSelectedRequest} onCopyCurl={() => void copyCurl()} canLoad={Boolean(selectedRun?.request)} />
