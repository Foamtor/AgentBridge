...
export function RequestComposer(props: Props) {
  ...
  return <section className="composer-zone">
    ...
    <div className="request-fields">
      <label><span>{copy.route}</span><select value={request.route} onChange={(event) => patch({ route: event.target.value, query: copy.routePrompt(event.target.value) })}>
    ...
    <p className="route-hint"><strong>{copy.pluginPurpose}：</strong>{selectedRoute?.description || copy.routeHint}</p>
    <label className="model-field"><span>{copy.model}</span><select value={request.model} onChange={(event) => patch({ model: event.target.value })}><option value="default">default · {props.models.find((model) => model.alias === "default")?.kind === "real" ? `${props.models.find((model) => model.alias === "default")?.model_name ?? "real model"} / ${copy.real}` : copy.fake}</option>{props.models.filter((model) => model.alias !== "default").map((model) => <option value={model.alias} key={model.alias}>{model.alias}{model.model_name ? ` · ${model.model_name}` : ""}{model.kind === "real" ? ` / ${copy.real}` : ` / ${copy.fake}`}</option>)}{request.model !== "default" && !props.models.some((model) => model.alias === request.model) ? <option value={request.model}>{request.model}</option> : null}</select></label>
    <label className="query-field"><span>{copy.query}<small>{copy.queryHint} · {copy.routePrompt(request.route)}</small></span><textarea rows={4} value={request.query} onChange={(event) => patch({ query: event.target.value })} onKeyDown={(event) => { if ((event.ctrlKey || event.metaKey) && event.key === "Enter" && !props.busy && request.query.trim()) { event.preventDefault(); props.onSend(); } if (event.key === "Escape" && props.busy) { event.preventDefault(); props.onCancel(); } }} /></label>
