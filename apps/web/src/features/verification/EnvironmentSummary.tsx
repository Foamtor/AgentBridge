...
type Bootstrap = {
  runtime?: { auth_mode?: string; llm_backend?: string; llm_mode?: string; llm_model?: string; real_model_ready?: boolean; knowledge_backend?: string };
...
export function EnvironmentSummary() {
  ...
  return <section className={`environment-summary environment-${status}`} aria-label={t("environment")}>
    ...
    {snapshot ? <div className="environment-facts"><span>{t("tenant")} <code>{snapshot.context?.tenant_id ?? "-"}</code></span><span>{t("data")} <code>{snapshot.reference?.data_class ?? "-"}</code></span><span>{snapshot.runtime?.llm_mode === "openai_compatible" ? `${snapshot.runtime.llm_model} · real` : t("fakeMode")}</span><span>{checkCount} checks</span></div> : <span className="muted">{t("offlineDescription")}</span>}
