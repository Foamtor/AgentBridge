...
export function useVerificationRun() {
  ...
  async function run() {
    ...
    try {
      await streamChatSse(streamUrl, {
        ...
        body: JSON.stringify({ query: definition.query, thread_id: threadId, route: "work_order_ops", extra: definition.extra }),
      }, {
  ...
  return { scenario, setScenario, threadId, verification, busy, error, run, cancel };
}
