...
export function VerificationReport({ events, question, token, scenario = "chart", onApprovalResolved }: Props) {
  const { t } = useI18n();
  ...
  const checks = evaluateScenario(scenario, events);
  const checkLabel = (check: ScenarioCheck) => {
    const labels: Record<ScenarioCheck["id"], "checkList" | "checkChart" | "checkCitation" | "checkDraft" | "checkApproval" | "checkCreatedOnce"> = {
      list: "checkList", chart: "checkChart", citation: "checkCitation", draft: "checkDraft", approval: "checkApproval", created_once: "checkCreatedOnce",
    };
  ...
  return <section className="verification-report" aria-label={t("verificationReport")}>
    ...
    {hasBusinessDisplay ? <section className="report-display"><h3>{t("structuredDisplay")}</h3><BusinessResults events={events} token={token} onPreset={() => undefined} onApprovalResolved={onApprovalResolved} showPresets={false} showHeading={false} /></section> : null}
