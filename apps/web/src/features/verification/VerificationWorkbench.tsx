...
export function VerificationWorkbench() {
  ...
  return <main className="page verification-workbench">
    ...
    <section className="workbench-section">
      ...
      <div className="run-actions"><button type="button" className="primary" onClick={() => void run.run()} disabled={run.busy || (run.mode === "real" && !testedModels.length)}>{run.busy ? t("running") : t("runScenario")}</button><button type="button" className="secondary" onClick={() => void run.cancel()} disabled={!run.busy}>{t("cancel")}</button>{modelsReady && !testedModels.length ? <Link className="playground-link" to="/models">{t("configureModel")}</Link> : null}</div>
    </section>
    {run.error ? <p className="error" role="alert">{run.error}</p> : null}
    <VerificationReport events={run.verification.events} question={run.query} scenario={run.scenario} token="" onApprovalResolved={run.reload} />
