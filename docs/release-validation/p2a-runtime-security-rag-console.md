...
> **状态：** 已完成（P2-A）。此记录只公开脱敏后的命令、版本、数量和结论；P2-B 的部署、具体 IdP 认证或多机验收属于 1.0 之后的后续工程。
...
P2-B 后续包括：干净环境部署、迁移与升级/回滚、备份恢复、具体身份提供方现场登录、厂商 external RAG 联调，以及双实例 Redis 锁和跨实例限流；这些不阻塞单机 1.0 发布。
...
- P2-B 后续项不计入 P2-A 完成，也不改变单机 1.0 的发布口径。
