...
> **状态：** 历史设计，核心能力已实现；当前首发范围以 [v0.1.0 Spec](./2026-08-01-p3a-open-source-release-readiness-design.md) 为准。
> **日期：** 2026-07-30  
...
默认体验使用仓库 migration 初始化的脱敏示例数据和 fake knowledge。RAG-Agent 已作为固定演示租户的**可选只读**后端完成集成，但不属于默认 Compose 依赖；不生成客户业务前端、不实现 OCR 或通用报表系统。
