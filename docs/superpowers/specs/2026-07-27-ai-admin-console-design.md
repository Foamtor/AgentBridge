...
> **状态：** 设计稿 v2.1（C0–C4 已在 `apps/web` 落地；见 [管理后端 final-spec](./2026-07-27-admin-backend-final-spec.md) §8）  
...
**C2+：** `/prompts`、配置档 A 可写。  
**C3+：** `/usage`。  
**C4+：** `/knowledge`（provider 未就绪时显示阻塞提示）。
...
| **C4** | + 知识后端 | 展示 status（503 阻塞态已处理）；入库任务待 R-B provider |

**实现分支：** `feat/admin-console-c0-c4`（`apps/web` 路由与侧栏已对齐上表）。
...
- [x] 集成方仅用控制台即可完成：选插件联调、看事件、看插件与配置只读  
- [x] 管理员能区分热配 / 启动项 / 密钥三档（配置页 tier 标识）  
- [x] 无 `admin:*` 无法写配置与 Prompt（RBAC 测试覆盖）  
- [x] 不出现业务前端信息架构  
