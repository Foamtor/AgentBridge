...
| | |
|--|--|
| 已有实现 | 流式对话、示例插件、权限骨架、单机运维、调试台、知识后端协议、AI 手册与**通用**提示词（可选 Cursor Skill） |
| 还在补 | 按需接入真实业务插件；贡献规范等外围文档 |
| 明确不做 | 云端 Studio、替代企业账号中心、默认无配置的多机集群 |

当前发布工作以「传统业务系统 + RAG」的黄金参考实现为中心：业务插件查询受权限约束的结构化数据，RAG 检索非结构化知识并返回引用，写操作可走审批。分期、验收门槛和发布边界见[发布规划](docs/release-plan.md)；各环境的支持条件见[部署支持矩阵](docs/deploy.md#支持矩阵与当前承诺)。
